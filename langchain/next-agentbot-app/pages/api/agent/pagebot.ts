  ...
  try {
    ...
    if (req.method == "POST") {
      ...
      const llm = new ChatOpenAI({
        ...
        temperature: 0,
        frequencyPenalty: 1.0, // 높은 패널티 설정
        presencePenalty: 1.0, // 높은 패널티 설정
      });

      //Step3:웹페이지 크롤링을 위한 웹페이지 로더 생성하기
      ...
      //Step5 : Splitting된 문자 내 단어들을 임베딩(벡터화처리) 처리하기 메모리 벡터저장소에 저장하기
      //const vectorStore = await MemoryVectorStore.fromDocuments(임베딩된 문서, 임베딩모델 처리기);
      ...
      //vector저자소 기반 검색기 변수 정의
      const retriever = vectorStore.asRetriever();
      const searchResult = await retriever.invoke(prompt);
      console.log("벡터저장소 검색결과 : ", searchResult);
