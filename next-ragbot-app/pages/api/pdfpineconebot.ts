...
//npm i @langchain/pinecone-database/pinecone
...
export default async function handler(
  ...
  try {
    ...
    if (req.method == "POST") {
      ...
      //Step3-3: Embeding/Embeding 수치데이터 저장 과정: Splitting된 문서내 단어들을 임베딩(벡터화처리)처리해서 pinecone벡터저장소에 저장하기
      ...
      //pineconeindex 객체 생성하기
      const pinecone = new PineconeClient();
      const pineconeIndex = pinecone.Index(process.env.PINECONE_INDEX!);

      //청크 데이터를 파인콘 지정 Index에 벡터 수치화 해서 저장하기
