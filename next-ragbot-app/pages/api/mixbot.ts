...
//OpenAI ChatGPT 객체 참조하기
import { ChatOpenAI } from "@langchain/openai";
//google gemini llm 객체 참조하기
import { ChatGoogleGenerativeAI } from "@langchain/google-genai";

//Gemini기반 RAG구현시 사용할 수 있는 구글 임베딩 모델참고
...
export default async function handler(
  ...
  try {
    ...
    if (req.method == "POST") {
      ...
      const botType = req.body.botType; //챗봇타입 추출
      ...
      // const response = await llm.invoke(message);
      // console.log("Gemini Bot : ", response);

      //case2 : 프롬프트 템플릿과 함께 gemini 챗봇 실행하기
      //   const outputParser = new StringOutputParser();
      //   const prompt = ChatPromptTemplate.fromMessages([
      //     ["system", "당신은 세계적으로 유명한 여행작가 입니다."],
      //     ["user", "{input}"],
      //   ]);
